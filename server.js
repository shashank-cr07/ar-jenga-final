...
const app = express();
const server = http.createServer(app);  //creating the server using express
const io = new Server(server, {
    cors: {
        origin: ["https://arjengaupdated1.vercel.app/", "https://localhost:5173"], //Allow vercel app url must be obtained after deploying on vercel
        methods: ["GET", "POST"],
    },
});
...
io.on('connection', (socket) => {
    ...
    socket.on('update-block', ({ roomId, blockData }) => {
        ...
        const playerBasePosition = player.playerData.basePosition;//finding the players base position so we can enable a relative change 
        if (!playerBasePosition) {
            console.warn(`Player ${socket.id} has no base position set.`);
            return;
        }
        // Calculate the relative movement since blocks can be loaded at different positions 
        const relativeChange = {
            ...
            quaternion: blockData.quaternion,  //maintaining orientation 
        };
...
const PORT = process.env.PORT || 3000;  //used to listen on the render app once deployed or on the localhost 
